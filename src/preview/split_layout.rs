...
pub(in crate::preview) const MIN_PANE_WIDTH_PX: f32 = 280.0;
...
pub(in crate::preview) fn left_width_for_viewport(viewport_width: f32, ratio: u16) -> f32 {
...
fn width_from_resize(session: ResizeSession, pointer_x: f32) -> f32 {
    ...
    (session.current_left_width + pointer_x - session.start_pointer_x).clamp(
        MIN_PANE_WIDTH_PX.min(content_width / 2.0),
        (content_width - MIN_PANE_WIDTH_PX).max(content_width / 2.0),
    )
}
...
impl WorkspaceWindow {
    pub(in crate::preview) fn rendered_left_pane_width(&self, viewport_width: f32) -> f32 {
    ...
    pub(in crate::preview) fn begin_split_resize(
        &mut self,
        pointer_x: f32,
        viewport_width: f32,
        cx: &mut Context<Self>,
    ...
    pub(in crate::preview) fn update_split_resize(
        &mut self,
        pointer_x: f32,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::preview) fn finish_split_resize(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::preview) fn cancel_split_resize(&mut self) -> bool {
