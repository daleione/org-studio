...
#let font-bold = (
  sys.inputs.at("theme-font-heading", default: "Rockwell"),
  "Hiragino Sans GB",
)
#let font-mono = (
  ...
  context {
    ...
    block(
      ...
      #grid(
        ...
        column-gutter: px(12),
        align: horizon,
        box(width: px(10), height: px(28), radius: px(5), fill: pick(i)), body,
      )
    ]
  }
}
  ...
  block(
    ...
    #stack(spacing: px(11), ..hs
      ...
      .map(((i, h)) => grid(
        ...
        column-gutter: px(11),
        align: horizon,
        box(width: px(24), height: px(24), radius: px(8), fill: pick(i), align(
          center + horizon,
          text(
            font: font-bold,
            size: tx(13),
...
#let divider() = align(center, block(above: px(34), below: px(34), stack(
  dir: ltr,
  spacing: px(10),
  ..range(5).map(i => circle(radius: px(5), fill: pick(i))),
)))
...
#let conf(doc) = {
  ...
  set list(
    marker: text(fill: color-sage, weight: 700)[•],
    body-indent: px(8),
    spacing: px(8),
  )
  set enum(
  ...
  set table(
    ...
    fill: (_, row) => if row == 0 { rgb(142, 189, 157, 30%) } else if calc.even(
      row,
    ) { color-zebra } else { white },
  )
  show table: it => block(
    width: 100%,
    radius: px(18),
