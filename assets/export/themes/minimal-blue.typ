...
#let theme-panel = rgb(sys.inputs.at("theme-color-panel", default: "#f6f5f4"))
#let theme-border = rgb(sys.inputs.at("theme-color-border", default: "#ebebeb"))
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#0075de"))
#let theme-accent = rgb(sys.inputs.at("theme-color-accent", default: "#0075de"))
...
#let color-ink    = rgb(0, 0, 0, 92%)
#let color-text   = rgb(0, 0, 0, 82%)
#let color-soft   = rgb(0, 0, 0, 62%)
...
#let color-hair   = rgb(0, 0, 0, 8%)
#let color-line   = rgb(0, 0, 0, 6%)
#let color-zebra  = rgb("#FAFAF9")
#let color-table-bd = rgb(0, 0, 0, 9%)
...
#let h1-block(body) = block(width: 100%, above: px(34), below: px(20))[
  #set text(font: hf(1, font-bold), size: tx(size-h1), weight: 700, fill: color-ink)
...
#let h2-block(body) = block(width: 100%, above: px(32), below: px(16),
  stroke: (left: px(3) + color-accent), inset: (left: px(12), y: px(2)))[
...
#let md-toc() = context {
  ...
  block(width: 100%, above: px(20), below: px(8))[
    #text(font: font-text, size: tx(13), weight: 600, fill: color-muted, tracking: 0.14em)[CONTENTS]
  ]
  block(width: 100%, below: px(16))[
    #set par(leading: 0.6em)
    #stack(spacing: px(12), ..hs.map(h => grid(
      columns: (auto, 1fr), column-gutter: px(12), align: horizon,
      box(width: px(15), height: px(15), radius: px(3),
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(34), width: 100%)[
  ...
  #grid(
    ...
    box(circle(radius: px(20), fill: color-chip2)[
      #align(center + horizon, text(font: font-bold, size: tx(18), weight: 600, fill: color-accent,
...
#let conf(doc) = {
  ...
  show strong: it => box(
    ...
    text(weight: 600, fill: color-accent, it.body),
  )
  ...
  show heading.where(level: 3): it => block(
    width: 100%, above: px(28), below: px(12),
  )[
  ...
  show link: it => text(fill: color-accent, weight: 500, it.body)
  ...
  show quote.where(block: true): it => block(
    ...
    #text(font: font-bold, size: tx(13), weight: 600, fill: color-accent, tracking: 0.15em)[QUOTE]
    #v(px(8))
    #set text(size: tx(16), fill: color-ink)
    #it.body
  ]

  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), fill: color-accent)
  ...
  set enum(
    numbering: n => box(
      width: px(22), height: px(22), radius: px(11), fill: color-chip2,
