...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#79ff5b"))
...
#let font-text  = (sys.inputs.at("theme-font-body", default: "Menlo"), "PingFang SC")
#let font-bold  = (sys.inputs.at("theme-font-heading", default: "Menlo"), "PingFang SC")
...
#let pixel-panel(body, fill: color-panel, border: color-bar, inset: px(18), above: px(24), below: px(24)) = block(
  width: 100%, above: above, below: below,
  fill: color-shadow,
)[
  #block(width: 100%, fill: fill, stroke: px(2) + border, inset: inset,
    outset: (bottom: 0pt, right: 0pt),
    radius: 0pt, body)
]

#let pixel-shadow-box(body, fill: color-panel, border: color-bar, inset: px(18), above: px(24), below: px(24)) = block(
  width: 100%, above: above, below: below)[
  #box(width: 100%, inset: (right: px(4), bottom: px(4)))[
    #box(width: 100%, fill: color-shadow)[
      #move(dx: -px(4), dy: -px(4),
        block(width: 100%, fill: fill, stroke: px(3) + border, radius: 0pt, inset: inset, body))
    ]
  ]
]

#let hairline(above: px(0), below: px(0)) = block(width: 100%, above: above, below: below,
  line(length: 100%, stroke: (paint: color-bar, thickness: px(2), dash: ("dot", px(3)))))

#let h1-block(body) = block(width: 100%, above: px(34), below: px(20))[
  #text(font: font-text, size: tx(15), weight: 500, fill: color-neon-dim, tracking: 0.18em)[>> SYSTEM]
  #v(px(6))
  #grid(columns: (auto, 1fr), column-gutter: px(10), align: horizon,
    text(font: font-bold, size: tx(34), weight: 500, fill: color-neon-dim, "#"),
    {
      set text(font: hf(1, font-bold), size: tx(size-h1), weight: 500, fill: color-green, tracking: 0.01em)
      set par(leading: 0.4em)
      body
    },
  )
  #v(px(14))
  #line(length: 100%, stroke: (paint: color-neon-dim, thickness: px(2), dash: ("dot", px(3))))
]

#let h2-block(body) = block(width: 100%, above: px(32), below: px(16))[
  #box(width: 100%, fill: color-bar, inset: (x: px(14), y: px(8)), radius: 0pt,
    stroke: (left: px(5) + color-neon))[
    #grid(columns: (auto, 1fr, auto), column-gutter: px(8), align: horizon,
      text(font: font-bold, size: tx(22), weight: 500, fill: color-neon, "["),
      {
        set text(font: hf(2, font-bold), size: tx(size-h2), weight: 500, fill: color-green, tracking: 0.04em)
        set par(leading: 0.4em)
        body
      },
      text(font: font-bold, size: tx(22), weight: 500, fill: color-neon, "]"),
...
#let md-toc() = context {
  ...
  block(width: 100%, above: px(20), below: px(8))[
    #text(font: font-text, size: tx(15), weight: 500, fill: color-magenta, tracking: 0.2em, "[## CONTENTS · CONTENTS]")
  ]
  pixel-shadow-box(border: color-neon, fill: color-panel2, below: px(16), inset: px(18))[
    #block(width: 100%, fill: color-neon, inset: (x: px(10), y: px(5)), radius: 0pt)[
      #text(font: font-text, size: tx(15), weight: 500, fill: color-shadow, tracking: 0.16em)[DIRECTORY/INDEX.SYS]
    ]
    #v(px(12))
    #set par(leading: 0.5em)
    #stack(spacing: px(12), ..hs.enumerate().map(((i, h)) => grid(
      columns: (auto, 1fr), column-gutter: px(10), align: horizon,
      text(font: font-mono, size: tx(15), fill: color-neon, weight: 700,
        std.numbering("01.", i + 1)),
      link(h.location(), text(font: font-text, fill: color-soft, weight: 500, size: tx(18), h.body)),
    )))
  ]
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(34), width: 100%)[
  ...
  #grid(
    ...
    box(fill: color-neon, inset: 0pt, radius: 0pt,
      box(width: px(40), height: px(40))[
        #align(center + horizon, text(font: font-bold, size: tx(24), weight: 500, fill: color-shadow,
          if author != "" { author.first() } else { "" }))
      ]),
    [
      #text(font: font-text, size: tx(18), fill: color-green, weight: 500)[#author] \
      #text(font: font-text, size: tx(15), fill: color-muted, tracking: 0.1em)[#date]
    ],
...
#let conf(doc) = {
  ...
  set text(font: font-text, fill: color-ink, size: tx(size-body), lang: "zh", weight: 500, tracking: 0.01em)
  set par(
  ...
  show strong: it => box(
    fill: color-neon, inset: (x: px(5), y: px(1)), radius: 0pt, outset: (y: px(2)),
  ...
  show heading.where(level: 3): it => block(width: 100%, above: px(28), below: px(12))[
    #set text(font: hf(3, font-bold), size: tx(size-h3), weight: 500, fill: color-neon-dim, tracking: 0.04em)
    #set par(leading: 0.4em)
    #text(fill: color-magenta, "### ") #it.body
  ]
  show heading.where(level: 4): it => block(above: px(24), below: px(10))[
    #set text(font: hf(4, font-bold), size: tx(size-h4), weight: 500, fill: color-green); #set par(leading: 0.4em)
    #text(fill: color-magenta, "#### ") #it.body
  ]
  show heading.where(level: 5): it => block(above: px(20), below: px(10))[
    #set text(font: hf(5, font-bold), size: tx(size-h5), weight: 500, fill: color-soft); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 6): it => block(above: px(18), below: px(10))[
    #set text(font: hf(6, font-bold), size: tx(size-h6), weight: 500, fill: color-muted); #set par(leading: 0.4em); #it.body
  ]
    ...
    if it.width == auto {
      layout(size => {
        ...
        align(center, box(fill: color-shadow)[
          #move(dx: -px(4), dy: -px(4),
            block(stroke: px(3) + color-bar, radius: 0pt, clip: true, inset: px(4), fill: color-panel2,
              { set image(width: w - px(8)); it }))
        ])
      })
    } else { it }
  }
  ...
  show quote.where(block: true): it => block(
    ...
    #text(font: font-text, size: tx(14), weight: 500, fill: color-neon-dim, tracking: 0.16em, "// QUOTE")
    #v(px(8))
    #set text(size: tx(18), fill: color-soft, style: "italic")
    #it.body
  ]

  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), fill: color-neon)
  show raw.where(block: false): box.with(
    fill: color-chip, inset: (x: px(5), y: px(1)), radius: 0pt, outset: (y: px(2)),
    stroke: px(1) + color-bar,
  )
  show raw.where(block: true): it => block(
    width: 100%, above: px(24), below: px(24), fill: color-shadow)[
    #move(dx: -px(4), dy: -px(4))[
      #block(width: 100%, fill: color-panel2, stroke: px(3) + color-bar, radius: 0pt, inset: 0pt)[
        #block(width: 100%, fill: color-bar, inset: (x: px(12), y: px(7)), radius: 0pt)[
          #grid(columns: (1fr, auto), align: horizon,
            text(font: font-mono, size: tx(12), fill: color-soft, tracking: 0.1em)[TERMINAL.SH],
            stack(dir: ltr, spacing: px(5),
  ...
  set list(marker: text(font: font-mono, fill: color-neon, weight: 700)[■], body-indent: px(8), spacing: px(8))
  set enum(
    numbering: n => box(inset: (right: px(2)),
      text(font: font-mono, size: tx(15), weight: 700, fill: color-neon-dim,
        std.numbering("01.", n))),
    body-indent: px(8), spacing: px(8),
  )

  set table(
    stroke: px(2) + color-bar, inset: (x: px(14), y: px(11)),
    fill: (_, row) => if row == 0 { color-bar } else if calc.odd(row) { color-zebra } else { color-panel2 },
  )
  show table: it => block(width: 100%, fill: color-shadow)[
    #move(dx: -px(4), dy: -px(4),
      block(width: 100%, radius: 0pt, clip: true, stroke: px(3) + color-bar, breakable: true, it))
  ]
