...
#let font-text = (
  sys.inputs.at("theme-font-body", default: "Courier New"),
  "Songti SC",
)
#let font-bold = (
...
// A brief cover sheet: the band bleeds to the page edges, the classification
// stamp rides on top of it, and the title is knocked out in white.
#let poster-title(title) = block(
  ...
  #box(
    stroke: px(1) + rgb(255, 255, 255, 45%),
    radius: px(3),
    inset: (x: px(7), y: px(3)),
    text(
      font: font-text,
...
#let conf(doc) = {
  ...
  set table(
    ...
    fill: (_, row) => if row == 0 { color-panel2 } else if calc.even(row) {
      color-zebra
    } else { none },
  )
  show table: it => block(
