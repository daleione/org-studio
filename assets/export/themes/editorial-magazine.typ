...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#ff2442"))
...
#let font-text  = (sys.inputs.at("theme-font-body", default: "Helvetica Neue"), "Songti SC", "PingFang SC")
#let font-bold  = (sys.inputs.at("theme-font-heading", default: "Helvetica Neue"), "Songti SC", "PingFang SC")
...
#let h1-block(body) = block(
  ...
  #set text(font: hf(1, font-bold), size: tx(size-h1), weight: 700, fill: color-text)
...
#let h2-block(body) = block(
  ...
  #box(
    ...
    #set text(font: hf(2, font-bold), size: tx(size-h2), weight: 600, fill: color-red)
...
#let md-toc() = context {
  ...
  block(width: 100%, below: px(16),
    fill: color-card, stroke: px(1) + color-border, radius: px(12),
    inset: (x: px(18), y: px(16)))[
    #set par(leading: 0.6em)
    #stack(spacing: px(11), ..hs.enumerate().map(((i, h)) => grid(
      columns: (auto, 1fr), column-gutter: px(12), align: horizon,
      text(font: font-label, fill: color-red, weight: 600, size: tx(14),
        "0" + str(i + 1)),
      link(h.location(), text(font: font-text, fill: color-text, weight: 500, size: tx(16), h.body)),
    )))
  ]
}
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(36), width: 100%)[
  ...
  #grid(
    ...
    box(circle(radius: px(24), fill: rgb(48, 48, 52, 8%), stroke: px(1) + color-border)[
      #align(center + horizon, text(font: font-bold, size: tx(20), weight: 600, fill: color-soft,
        if author != "" { author.first() } else { "" }))
    ]),
    [
      #text(font: font-bold, size: tx(17), fill: color-text, weight: 700)[#author] \
      #text(font: font-label, size: tx(13), fill: color-muted, tracking: 0.1em)[#date]
    ],
...
#let divider() = align(center, block(above: px(38), below: px(38))[
    #grid(columns: (1fr, auto, 1fr), column-gutter: px(14), align: horizon,
      line(length: 100%, stroke: px(1) + color-line),
...
#let conf(doc) = {
  ...
  show strong: it => text(font: font-bold, weight: 700, fill: color-red, it.body)
  ...
  show heading.where(level: 3): it => block(
    ...
    stroke: (left: px(4) + color-red), inset: (left: px(12), y: 2pt),
  )[
  ...
  show heading.where(level: 4): it => block(above: px(24), below: px(12))[
    #set text(font: hf(4, font-bold), size: tx(size-h4), weight: 700, fill: color-text); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 5): it => block(above: px(20), below: px(10))[
    #set text(font: hf(5, font-bold), size: tx(size-h5), weight: 700, fill: color-soft); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 6): it => block(above: px(18), below: px(10))[
    #set text(font: hf(6, font-bold), size: tx(size-h6), weight: 700, fill: color-muted); #set par(leading: 0.4em); #it.body
  ]
  ...
  show quote.where(block: true): it => block(width: 100%, above: px(28), below: px(28),
    fill: color-card, stroke: px(1) + color-border, radius: px(14),
    inset: (left: px(22), right: px(20), top: px(30), bottom: px(20)))[
    #place(top + left, dx: px(10), dy: px(-18),
      text(font: ("Songti SC",), size: tx(58), weight: 700, fill: color-red-bg)["])
  ...
  show raw.where(block: false): box.with(
    fill: color-red-bg, inset: (x: px(5), y: px(1)), radius: px(4), outset: (y: px(2)),
  )
  show raw.where(block: true): it => block(
    width: 100%, above: px(24), below: px(24),
    fill: color-card, stroke: px(1) + color-border, radius: px(10), clip: true,
  )[
  ...
  set list(marker: text(fill: color-red, weight: 700)[•], body-indent: px(8), spacing: px(8))
  set enum(
    numbering: n => box(
      width: px(22), height: px(22), radius: px(11), fill: color-red,
      align(center + horizon, text(font: font-label, size: tx(13), weight: 700, fill: white, str(n))),
    ),
  ...
  set table(
    ...
    fill: (_, row) => if row == 0 { color-red-bg } else if calc.even(row) { color-zebra } else { none },
  )
  ...
  show table.cell.where(y: 0): set text(font: font-bold, weight: 700, fill: color-red)
