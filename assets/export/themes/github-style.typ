...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#0969da"))
...
#let divider() = align(center, block(
    width: 100%, height: px(1), above: px(34), below: px(34),
    fill: gradient.linear(
      (rgb("#d1d9e000"), 0%), (color-table-bd, 16%), (color-accent, 50%),
      (color-table-bd, 84%), (rgb("#d1d9e000"), 100%), dir: ltr),
  ))
...
#let conf(doc) = {
  ...
  show strong: it => highlight(
    fill: rgb(9, 105, 218, 14%), top-edge: 0.32em, bottom-edge: -0.18em, extent: px(2),
  ...
  show heading.where(level: 3): it => block(
    ...
    stroke: (left: px(3) + color-accent2), inset: (left: px(12), y: 2pt),
  )[
    #set text(font: hf(3, font-bold), size: tx(size-h3), weight: 600, fill: color-text)
    #set par(leading: 0.4em)
    #it.body
  ]
  show heading.where(level: 4): it => block(above: px(24), below: px(12))[
    #set text(font: hf(4, font-text), size: tx(size-h4), weight: 600, fill: color-text); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 5): it => block(above: px(20), below: px(10))[
    #set text(font: hf(5, font-text), size: tx(size-h5), weight: 600, fill: color-muted); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 6): it => block(above: px(18), below: px(10))[
    #set text(font: hf(6, font-text), size: tx(size-h6), weight: 600, fill: color-muted); #set par(leading: 0.4em); #it.body
  ]
  ...
  show quote.where(block: true): it => block(
    width: 100%, above: px(18), below: px(18),
    fill: color-panel,
    stroke: (left: px(4) + color-accent2, rest: px(1) + color-border),
    radius: (right: px(10)),
    inset: (left: px(16), right: px(16), y: px(14)),
    { set text(size: tx(16), fill: color-text); it.body },
  )

  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), fill: color-text)
  show raw.where(block: true): it => block(
    width: 100%, above: px(16), below: px(16),
    fill: color-panel, radius: px(6), clip: true,
  )[
    #block(width: 100%, height: px(36), fill: rgb("#eef1f4"), inset: (x: px(12)))[
  ...
  set enum(
    numbering: n => box(
      width: px(22), height: px(22), radius: px(11), fill: rgb(9, 105, 218, 10%),
  ...
  set table(
    stroke: px(1) + color-table-bd, inset: (x: px(16), y: px(8)),
