...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#001f3f"))
...
#let hairline(above: px(0), below: px(0)) = block(width: 100%, above: above, below: below,
  line(length: 100%, stroke: (paint: color-hair, thickness: px(1), dash: "dashed")))

#let h1-block(body) = block(width: 100%, above: px(34), below: px(22))[
  #box(fill: color-rust, inset: (x: px(8), y: px(3)), radius: px(2),
    text(font: font-mono, size: tx(11), weight: 700, fill: white, tracking: 0.18em, "PUBLISHED"))
  #v(px(14))
  #grid(columns: (auto, 1fr), column-gutter: px(14), align: top,
    box(width: px(6), height: px(40), fill: color-rust-hi),
    {
      set text(font: hf(1, font-serif), size: tx(size-h1), weight: 700, fill: color-navy, tracking: -0.005em)
...
#let md-toc() = context {
  ...
  block(width: 100%, above: px(20), below: px(8))[
    #text(font: font-mono, size: tx(12), weight: 700, fill: color-rust, tracking: 0.2em, "CONTENTS · CONTENTS")
  ]
  block(width: 100%, below: px(16),
    fill: color-paper, stroke: px(2) + color-navy, inset: px(18))[
    #set par(leading: 0.6em)
    #stack(spacing: px(12), ..hs.enumerate().map(((i, h)) => grid(
      columns: (auto, 1fr), column-gutter: px(12), align: horizon,
      text(font: font-mono, size: tx(13), weight: 700, fill: color-rust,
        "0" + str(i + 1)),
      link(h.location(), text(font: font-text, fill: color-navy, weight: 500, size: tx(16), h.body)),
    )))
  ]
  ...
  #block(width: 100%, fill: color-navy, inset: px(20))[
    #grid(
      ...
      box(fill: color-rust, width: px(40), height: px(40))[
        #align(center + horizon, text(font: font-serif, size: tx(20), weight: 700, fill: white,
          if author != "" { author.first() } else { "" }))
      ],
      [
        #text(font: font-text, size: tx(16), fill: white, weight: 600)[#author] \
        #text(font: font-mono, size: tx(12), fill: rgb(248, 249, 250, 60%), tracking: 0.1em)[#upper(date)]
      ],
    ...
    #v(px(10))
    #text(font: font-mono, size: tx(11), weight: 700, fill: color-rust-hi, tracking: 0.24em, "MARKDOWNREADER")
  ]
]
...
#let conf(doc) = {
  ...
  show strong: it => box(
    fill: color-chip, inset: (x: px(5), y: px(1)), radius: px(2), outset: (y: px(2)),
  ...
  show heading.where(level: 3): it => block(width: 100%, above: px(28), below: px(12))[
    #set text(font: hf(3, font-serif), size: tx(size-h3), weight: 700, fill: color-rust)
    #set par(leading: 0.42em)
    #it.body
  ]
  show heading.where(level: 4): it => block(above: px(24), below: px(10))[
    #set text(font: hf(4, font-serif), size: tx(size-h4), weight: 700, fill: color-navy); #set par(leading: 0.42em); #it.body
  ]
  ...
  show heading.where(level: 6): it => block(above: px(18), below: px(10))[
    #set text(font: hf(6, font-mono), size: tx(size-h6), weight: 700, fill: color-muted, tracking: 0.16em); #set par(leading: 0.42em); #upper(it.body)
  ]
  ...
  show image: it => {
    if it.width == auto {
      layout(size => {
        ...
        align(center, block(stroke: px(2) + color-navy, inset: px(5), fill: white,
          { set image(width: w); it }))
      })
    } else { it }
  }

  show link: it => underline(offset: px(3), stroke: px(1) + color-rust,
    text(fill: color-rust, weight: 600, it.body))
  ...
  show quote.where(block: true): it => block(
    ...
    #grid(columns: (auto, 1fr), column-gutter: px(8), align: horizon,
      text(font: font-bold, size: tx(34), weight: 700, fill: color-rust, tracking: 0em)[“],
      text(font: font-mono, size: tx(11), weight: 700, fill: color-rust, tracking: 0.16em)[QUOTE],
    )
    #v(px(6))
  ...
  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), weight: 600, fill: color-rust)
  ...
  set list(marker: text(font: font-mono, fill: color-rust, weight: 700)[—], body-indent: px(8), spacing: px(8))
  set enum(
    numbering: n => box(
      width: px(22), height: px(22), fill: color-navy,
      align(center + horizon, text(font: font-mono, size: tx(13), weight: 700, fill: color-paper, str(n))),
    ),
  ...
  set table(
    ...
    fill: (_, row) => if row == 0 { color-paper } else if calc.even(row) { color-zebra } else { none },
  )
  show table: it => block(width: 100%, clip: true,
    stroke: px(2) + color-navy, breakable: true, it)
  show table: set text(size: tx(15), fill: color-ink)
  show table.cell.where(y: 0): set text(font: font-mono, weight: 700, fill: color-rust)
