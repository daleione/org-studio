...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#ffb5a0"))
...
#let h1-block(body) = block(width: 100%, above: px(34), below: px(20))[
  #set text(font: hf(1, font-bold), size: tx(size-h1), weight: 800, tracking: -0.02em,
...
#let h2-block(body) = block(width: 100%, above: px(34), below: px(18))[
  #grid(columns: (auto, 1fr), column-gutter: px(12), align: horizon,
    ...
    {
      set text(font: hf(2, font-bold), size: tx(size-h2), weight: 800, fill: color-peach, tracking: 0.02em)
      set par(leading: 0.40em)
      body
    },
  )
...
#let md-toc() = context {
  ...
  block(width: 100%, above: px(20), below: px(8))[
    #text(font: font-mono, size: tx(12), weight: 500, fill: color-volt, tracking: 0.2em, "INDEX // CONTENTS")
  ]
  block(width: 100%, below: px(16),
    fill: color-panel, stroke: px(1) + color-hair, inset: px(18))[
    #set par(leading: 0.6em)
    #stack(spacing: px(12), ..hs.map(h => grid(
      columns: (auto, 1fr), column-gutter: px(12), align: horizon,
      box(width: px(13), height: px(13), fill: none, stroke: px(2) + color-volt),
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(34), width: 100%)[
  ...
  #grid(
    ...
    box(width: px(40), height: px(40), fill: color-panel, stroke: px(2) + color-volt)[
      #align(center + horizon, text(font: font-bold, size: tx(18), weight: 800, fill: color-peach,
        if author != "" { author.first() } else { "" }))
    ],
    [
      #text(font: font-text, size: tx(16), fill: color-ink, weight: 600)[#author] \
      #text(font: font-mono, size: tx(13), fill: color-muted, tracking: 0.05em)[#date]
    ],
...
#let divider() = block(width: 100%, above: px(34), below: px(34),
    grid(columns: (px(40), 1fr), column-gutter: px(10), align: horizon,
      line(length: 100%, stroke: px(2) + color-volt),
      line(length: 100%, stroke: px(1) + color-hair)))
...
#let conf(doc) = {
  ...
  show strong: it => box(
    fill: color-chip, inset: (x: px(5), y: px(1)), outset: (y: px(2)),
  ...
  show heading.where(level: 4): it => block(above: px(24), below: px(10))[
    #set text(font: hf(4, font-text), size: tx(size-h4), weight: 600, fill: color-ink); #set par(leading: 0.40em); #it.body
  ]
  show heading.where(level: 5): it => block(above: px(20), below: px(10))[
    #set text(font: hf(5, font-text), size: tx(size-h5), weight: 600, fill: color-soft); #set par(leading: 0.40em); #it.body
  ]
  show heading.where(level: 6): it => block(above: px(18), below: px(10))[
    #set text(font: hf(6, font-mono), size: tx(size-h6), weight: 500, fill: color-muted, tracking: 0.1em); #set par(leading: 0.40em); #it.body
  ]
  ...
  show quote.where(block: true): it => block(
    ...
    #text(font: font-mono, size: tx(11), weight: 500, fill: color-volt, tracking: 0.2em, "// QUOTE")
    #v(px(8))
  ...
  show raw.where(block: false): box.with(
    fill: color-panel, inset: (x: px(5), y: px(1)), outset: (y: px(2)), stroke: px(1) + color-hair,
  )
  show raw.where(block: true): it => block(
    ...
    #block(below: px(12))[
      #text(font: font-mono, size: tx(11), weight: 500, fill: color-volt, tracking: 0.2em, "// CODE")
    ]
  ...
  set list(marker: text(fill: color-volt, weight: 700)[▸], body-indent: px(8), spacing: px(8))
  set enum(
    numbering: n => box(
      width: px(24), height: px(24), fill: color-panel, stroke: px(1) + color-volt,
      align(center + horizon, text(font: font-mono, size: tx(13), weight: 700, fill: color-peach, str(n))),
    ),
  ...
  set table(
    ...
    fill: (_, row) => if row == 0 { color-panel3 } else if calc.even(row) { color-zebra } else { none },
  )
  show table: it => block(width: 100%, clip: true,
    stroke: px(1) + color-hair, breakable: true, it)
  show table: set text(size: tx(15), fill: color-soft)
  show table.cell.where(y: 0): set text(font: font-mono, weight: 700, fill: color-peach, tracking: 0.08em)
