...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#001e73"))
...
#let size-h1      = float(sys.inputs.at("theme-size-h1",      default: "33"))
...
#let cutout(w, h, fill, dx: px(0), dy: px(0), rot: 0deg) = place(dx: dx, dy: dy,
  rotate(rot, box(width: w, height: h, fill: fill,
    radius: (top-left: px(18), top-right: px(7), bottom-right: px(20), bottom-left: px(6)))))
...
#let h1-block(body) = block(width: 100%, above: px(34), below: px(22))[
  ...
  #v(px(8))
  #set text(font: hf(1, font-serif), size: tx(size-h1), weight: 700, tracking: -0.01em, fill: color-navy)
...
#let h2-block(body) = block(width: 100%, above: px(34), below: px(16))[
  #grid(columns: (auto, 1fr), column-gutter: px(13), align: horizon,
    box(width: px(13), height: px(28), fill: color-orange,
      radius: (top-left: px(8), top-right: px(2), bottom-right: px(9), bottom-left: px(3))),
    {
      set text(font: hf(2, font-serif), size: tx(size-h2), weight: 700, fill: color-navy)
      set par(leading: 0.42em)
      body
    },
  )
...
#let md-toc() = context {
  ...
  block(width: 100%, above: px(22), below: px(10))[
    #text(font: font-text, size: tx(13), weight: 700, fill: color-navy, tracking: 0.2em)[CUTOUT CONTENTS · CONTENTS]
  ]
  block(width: 100%, below: px(18))[
    #box(width: 100%)[
      #place(dx: px(8), dy: px(8), block(width: 100%, height: 100%, radius: px(10), fill: color-navy))
      #block(width: 100%, fill: color-card, stroke: px(2) + color-navy, radius: px(10), inset: px(20))[
        #set par(leading: 0.6em)
        #stack(spacing: px(13), ..hs.enumerate().map(((i, h)) => grid(
          columns: (auto, 1fr), column-gutter: px(12), align: horizon,
          box(width: px(15), height: px(15), fill: dots.at(calc.rem(i, dots.len())),
            radius: (top-left: px(7), top-right: px(2), bottom-right: px(8), bottom-left: px(2))),
          link(h.location(), text(fill: color-navy-dk, weight: 600, size: tx(16), h.body)),
        )))
      ]
    ]
  ]
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(34), width: 100%)[
  ...
  #grid(
    ...
    box(rotate(-4deg, box(width: px(42), height: px(42), fill: color-orange,
      stroke: px(2) + color-navy,
      radius: (top-left: px(16), top-right: px(5), bottom-right: px(18), bottom-left: px(5)))[
      #align(center + horizon, text(font: font-bold, size: tx(18), weight: 700, fill: color-navy,
...
#let divider() = block(width: 100%, above: px(34), below: px(34),
    grid(columns: (px(40), px(26), px(20), 1fr), rows: px(5), column-gutter: px(7),
      box(fill: color-orange, width: 100%, height: 100%, radius: (top-left: px(4), bottom-right: px(5))),
      box(fill: color-magenta, width: 100%, height: 100%, radius: (top-left: px(4), bottom-right: px(5))),
      box(fill: color-emerald, width: 100%, height: 100%, radius: (top-left: px(4), bottom-right: px(5))),
...
#let conf(doc) = {
  ...
  show strong: it => box(
    fill: color-orangetint, inset: (x: px(5), y: px(1)), outset: (y: px(2)),
    radius: (top-left: px(6), top-right: px(2), bottom-right: px(7), bottom-left: px(2)),
  ...
  show heading.where(level: 3): it => block(width: 100%, above: px(28), below: px(12))[
    #set text(font: hf(3, font-bold), size: tx(size-h3), weight: 700, fill: color-cobalt)
    #set par(leading: 0.42em)
    #it.body
  ]
  show heading.where(level: 4): it => block(above: px(24), below: px(10))[
    #set text(font: hf(4, font-text), size: tx(size-h4), weight: 600, fill: color-navy); #set par(leading: 0.42em); #it.body
  ]
    ...
    if it.width == auto {
      layout(size => {
        ...
        align(center, box(width: w)[
          #place(dx: px(8), dy: px(8), block(width: 100%, height: 100%, radius: px(6), fill: color-navy))
  ...
  show quote.where(block: true): it => block(
    ...
    #v(px(2))
    #set text(font: font-serif, size: tx(17), fill: color-navy-dk, style: "italic", weight: 500)
    #it.body
  ]

  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), fill: color-rust)
  show raw.where(block: false): box.with(
    fill: color-card2, inset: (x: px(5), y: px(1)), radius: px(4), outset: (y: px(2)),
  )
  ...
  set enum(
    numbering: n => box(width: px(22), height: px(22), fill: color-navy,
      radius: (top-left: px(10), top-right: px(3), bottom-right: px(11), bottom-left: px(3)),
      align(center + horizon, text(size: tx(13), weight: 700, fill: color-bg, str(n)))),
    body-indent: px(10), spacing: px(8),
  )

  set table(
    stroke: px(1) + color-navy, inset: (x: px(16), y: px(12)),
    fill: (_, row) => if row == 0 { color-navy } else if calc.even(row) { color-card } else { none },
  )
  show table: it => block(width: 100%, radius: px(8), clip: true,
    stroke: px(2) + color-navy, breakable: true, it)
