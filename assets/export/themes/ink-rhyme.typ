...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#000000"))
...
#let h2-block(body) = block(
  ...
  stroke: (left: px(3) + color-ink), inset: (left: px(12), y: 2pt),
)[
...
#let md-toc() = context {
  ...
  block(width: 100%, below: px(16))[
    ...
    #stack(spacing: px(8), ..hs.map(h => grid(
      ...
      link(h.location(), underline(stroke: 0.5pt + color-ink, offset: 2pt,
        text(fill: color-ink, size: tx(15), h.body))),
    )))
  ]
}
...
#let divider() = align(center, block(
    width: 34%, height: px(1), above: px(38), below: px(38),
    fill: gradient.linear(
      (rgb(0, 0, 0, 0%), 0%), (rgb(0, 0, 0, 88%), 28%),
      (rgb(0, 0, 0, 88%), 72%), (rgb(0, 0, 0, 0%), 100%), dir: ltr),
  ))
...
#let conf(doc) = {
  ...
  show heading.where(level: 3): it => block(above: px(28), below: px(14), inset: (bottom: px(6)))[
  ...
  show heading.where(level: 6): it => block(above: px(18), below: px(10))[
    #set text(font: hf(6, font-text), size: tx(size-h6), weight: 600, fill: rgb("#888888")); #set par(leading: 0.4em); #it.body
  ]
  ...
  show link: it => underline(stroke: 0.5pt + color-ink, offset: 2pt, text(fill: color-ink, it.body))
  ...
  show raw.where(block: true): it => block(
    width: 100%, above: px(24), below: px(24),
    fill: color-panel, radius: px(2), clip: true, stroke: px(1) + color-border,
  )[
    #block(width: 100%, height: px(36), fill: rgb("#e8e8e8"), inset: (x: px(12)))[
  ...
  set enum(
    numbering: n => box(
      width: px(22), height: px(22), radius: px(11), fill: rgb("#f0f0f0"),
      stroke: px(1) + color-table-bd,
      align(center + horizon, text(size: tx(13), weight: 700, fill: color-ink, str(n))),
    ),
