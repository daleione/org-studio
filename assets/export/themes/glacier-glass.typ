...
#let theme-ink = rgb(sys.inputs.at("theme-color-ink", default: "#e0e8f0"))
#let theme-muted = rgb(sys.inputs.at("theme-color-muted", default: "#a0b4c4"))
#let theme-bg = rgb(sys.inputs.at("theme-color-bg", default: "#0a0e1a"))
#let theme-panel = rgb(sys.inputs.at("theme-color-panel", default: "#16203a"))
#let theme-border = rgb(sys.inputs.at("theme-color-border", default: "#2a3348"))
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#7dd3fc"))
#let theme-accent = rgb(sys.inputs.at("theme-color-accent", default: "#c8a0f0"))
...
#let color-ice    = theme-primary
#let color-ice-hi = rgb("#c8eaff")
#let color-laven  = theme-accent
#let color-ink    = theme-ink
#let color-soft   = theme-muted
#let color-muted  = rgb(160, 180, 196, 55%)
#let color-glass  = rgb(26, 36, 56, 55%)
#let color-accent-bg = rgb(125, 211, 252, 6%)
#let color-accent-bd = rgb(125, 211, 252, 22%)
#let color-hair   = rgb(255, 255, 255, 10%)
#let color-line   = rgb(255, 255, 255, 6%)
#let color-chip   = rgb(125, 211, 252, 12%)
#let color-zebra  = rgb(255, 255, 255, 3%)
#let color-table-bd = rgb(255, 255, 255, 9%)
...
#let h1-block(body) = block(width: 100%, above: px(34), below: px(20))[
  #set text(font: hf(1, font-bold), size: tx(size-h1), weight: 800, tracking: -0.01em,
    fill: gradient.linear(color-ice, color-ice-hi, angle: 20deg))
...
#let h2-block(body) = block(width: 100%, above: px(32), below: px(16))[
  #grid(columns: (auto, 1fr), column-gutter: px(12), align: horizon,
    box(width: px(24), height: px(5), radius: px(3), fill: color-ice),
    {
...
#let md-toc() = context {
  ...
  block(width: 100%, above: px(20), below: px(8))[
    #text(font: font-text, size: tx(13), weight: 600, fill: color-ice, tracking: 0.16em)[CONTENTS · CONTENTS]
  ]
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(34), width: 100%)[
  ...
  #grid(
    ...
    box(circle(radius: px(20), fill: color-chip, stroke: px(1) + color-accent-bd)[
      #align(center + horizon, text(font: font-bold, size: tx(18), weight: 700, fill: color-ice,
...
#let divider() = block(width: 100%, above: px(34), below: px(34),
    line(length: 100%, stroke: px(1) + gradient.linear(
      rgb(255, 255, 255, 0%), color-hair, rgb(255, 255, 255, 0%))))
...
#let conf(doc) = {
  set page(
    ...
    fill: gradient.radial(color-bg2, color-bg, center: (22%, 6%), radius: 110%),
    footer: if paged {
  ...
  show strong: it => box(
    ...
    text(weight: 700, fill: color-ice, it.body),
  )
  show emph: it => text(style: "italic", fill: color-laven, it.body)
  ...
  show heading.where(level: 3): it => block(width: 100%, above: px(28), below: px(12))[
    #set text(font: hf(3, font-bold), size: tx(size-h3), weight: 700, fill: color-ice)
    #set par(leading: 0.42em)
    #it.body
  ]
  ...
  show link: it => text(fill: color-ice, weight: 500, it.body)

  show quote.where(block: true): it => block(
    width: 100%, above: px(26), below: px(26),
    fill: color-accent-bg,
    stroke: (left: px(3) + color-ice, rest: px(1) + color-line),
    radius: (right: px(10)),
    inset: (left: px(22), rest: px(18)),
  )[
    #grid(columns: (auto, 1fr), column-gutter: px(8), align: horizon,
      text(font: font-bold, size: tx(34), weight: 700, fill: color-ice, tracking: 0em)[“],
      text(font: font-bold, size: tx(11), weight: 600, fill: color-ice, tracking: 0.16em)[QUOTE],
    )
    #v(px(6))
  ...
  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), fill: color-ice)
  ...
  set list(marker: text(fill: color-ice, weight: 700)[•], body-indent: px(8), spacing: px(8))
  set enum(
    numbering: n => box(
      width: px(22), height: px(22), radius: px(11), fill: color-chip,
      align(center + horizon, text(size: tx(13), weight: 700, fill: color-ice, str(n))),
    ),
  ...
  show table.cell.where(y: 0): set text(weight: 700, fill: color-ice)
