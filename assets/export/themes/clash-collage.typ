...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#fa2d1a"))
...
#let font-bold  = (sys.inputs.at("theme-font-heading", default: "Helvetica Neue"), "PingFang SC", "PingFang SC", "PingFang SC")
...
#let kicker(label, fill: color-red, angle: -3deg) = block(above: px(4), below: px(14))[
  #tilt(angle, box(
    fill: fill, inset: (x: px(14), y: px(8)),
  )[
    #set text(font: font-bold, size: tx(13), weight: 900, fill: white, tracking: 0.22em)
    #label
  ])
]

#let h1-block(body) = block(width: 100%, above: px(34), below: px(22), layout(size => {
  let w = size.width
  let inner = block(
    width: w - px(8),
    fill: color-ink,
    ...
    #set text(font: hf(1, font-bold), size: tx(size-h1), weight: 900, fill: color-bg, tracking: 0.01em)
  ...
  context {
    ...
    block(width: s.width + px(8), height: s.height + px(8))[
      #place(top + left, dx: px(8), dy: px(8),
        rect(width: s.width, height: s.height, fill: color-red))
  ...
  context {
    ...
    block(width: 100%, above: px(36), below: px(20))[
      #tilt(angle, block(
        ...
        #set text(font: hf(2, font-bold), size: tx(size-h2), weight: 900, fill: white, tracking: 0.04em)
        #set par(leading: 0.42em)
        #grid(columns: (auto, 1fr), column-gutter: px(12), align: horizon,
          box(rect(width: px(10), height: px(26), fill: color-bg)),
          body,
        )
      ])
    ]
...
#let md-toc() = context {
  ...
  let toc-item(i, h) = {
    ...
    let inner = box(
      ...
      fill: color-panel, stroke: px(2) + c,
      inset: (left: px(10), right: px(14), y: px(8)),
      grid(columns: (auto, 1fr), column-gutter: px(10), align: horizon,
        box(fill: c, inset: (x: px(7), y: px(3)),
          text(font: font-bold, size: tx(13), weight: 900, fill: white, str(i + 1))),
        link(h.location(), text(fill: color-ink, weight: 700, size: tx(16), h.body)),
      ),
    )
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(36), width: 100%)[
  #grid(
    ...
    tilt(-3deg, box(fill: color-ink, inset: px(12))[
      #align(center + horizon, text(font: font-bold, size: tx(20), weight: 900, fill: color-bg,
...
#let divider() = block(width: 100%, above: px(36), below: px(36),
    grid(columns: (1fr, 1fr), rows: px(5),
...
#let conf(doc) = {
  ...
  show strong: it => box(
    fill: color-red, inset: (x: px(5), y: px(1)), outset: (y: px(2)),
  ...
  show heading.where(level: 3): it => block(
    ...
    stroke: (left: px(5) + color-blue), inset: (left: px(12), y: 2pt),
  )[
    #set text(font: hf(3, font-bold), size: tx(size-h3), weight: 900, fill: color-ink)
    #set par(leading: 0.4em)
    #it.body
  ]
  show heading.where(level: 4): it => block(above: px(24), below: px(12))[
    #set text(font: hf(4, font-bold), size: tx(size-h4), weight: 700, fill: color-red); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 5): it => block(above: px(20), below: px(10))[
    #set text(font: hf(5, font-text), size: tx(size-h5), weight: 700, fill: color-blue); #set par(leading: 0.4em); #it.body
  ]
  ...
  show quote.where(block: true): it => block(width: 100%, above: px(28), below: px(28))[
    #block(width: 100%, fill: color-ink, inset: (x: px(22), y: px(20)))[
      #text(font: font-bold, size: tx(13), weight: 900, fill: color-red, tracking: 0.2em)[QUOTE] \
      #v(px(10))
  ...
  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), fill: color-red)
  show raw.where(block: false): box.with(
    fill: color-panel, stroke: px(1.5) + color-red,
    inset: (x: px(5), y: px(1)), outset: (y: px(2)),
  )
  show raw.where(block: true): it => block(
    width: 100%, above: px(24), below: px(24),
    fill: color-panel, stroke: px(2.5) + color-ink, clip: true,
  )[
    #block(width: 100%, height: px(8), fill: gradient.linear(
      (color-red, 0%), (color-red, 50%), (color-blue, 50%), (color-blue, 100%), dir: ltr))
  ...
  set list(marker: box(fill: color-red, inset: px(3), outset: (y: px(1))), body-indent: px(8), spacing: px(8))
  set enum(
    numbering: n => box(
      width: px(22), height: px(22),
      fill: if calc.odd(n) { color-red } else { color-blue },
      align(center + horizon, text(font: font-bold, size: tx(13), weight: 900, fill: white, str(n))),
    ),
    body-indent: px(10), spacing: px(8),
  )

  set table(
    stroke: px(2) + color-ink, inset: (x: px(16), y: px(12)),
    fill: (_, row) => if row == 0 { color-blue } else if calc.even(row) { color-zebra } else { none },
  )
  show table: it => block(width: 100%, clip: true, stroke: px(2.5) + color-ink, breakable: true, it)
  show table: set text(size: tx(15), fill: color-ink)
  show table.cell.where(y: 0): set text(weight: 900, fill: white, font: font-bold)
