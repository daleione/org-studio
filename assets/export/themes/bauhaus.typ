...
#let theme-primary = rgb(sys.inputs.at("theme-color-primary", default: "#e30613"))
#let theme-accent  = rgb(sys.inputs.at("theme-color-accent",  default: "#004d9f"))

#let size-h1      = float(sys.inputs.at("theme-size-h1",      default: "31"))
...
#let font-heavy = (sys.inputs.at("theme-font-heading", default: "Helvetica Neue"), "PingFang SC", "PingFang SC")
...
#let h1-block(body) = block(width: 100%, above: px(36), below: px(18), layout(size => {
  let card = block(
    width: size.width - px(6),
    fill: gradient.linear(
      (color-red, 0%), (color-red, 64%), (color-blue, 64%),
      (color-blue, 86%), (color-yellow, 86%), (color-yellow, 100%), dir: ltr),
    stroke: px(3) + color-ink,
    inset: (left: px(14), right: px(14), top: px(10), bottom: px(9)),
  )[
    #set text(font: hf(1, font-heavy), size: tx(size-h1), weight: 800, fill: white)
    #set par(leading: 0.3em)
    #upper(body)
  ]
  let h = measure(card).height
  place(dx: px(6), dy: px(6), rect(width: size.width - px(6), height: h, fill: color-ink))
  card
}))
...
#let h2-block(body) = block(
  ...
  fill: gradient.linear(
    (color-yellow, 0%), (color-yellow, 68%), (white, 68%), (white, 100%), dir: ltr),
  stroke: (left: px(10) + color-blue, top: px(3) + color-ink, bottom: px(3) + color-ink),
  inset: (x: px(16), y: px(8)),
)[
  #set text(font: hf(2, font-heavy), size: tx(size-h2), weight: 800, fill: color-ink, tracking: 0.08em)
...
#let md-toc() = context {
  ...
  block(width: 100%, below: px(16))[
    ...
    #stack(spacing: px(8), ..hs.map(h => grid(
      ...
      link(h.location(), text(fill: color-blue, size: tx(15), weight: 600, h.body)),
    )))
  ]
}
...
#let signature-row(author: "", date: "", logo: logo-url) = block(above: px(34), width: 100%)[
  #grid(
    ...
    [
      #text(font: font-bold, size: tx(15), fill: color-ink, weight: 700)[#author] \
...
#let divider() = block(
    width: 100%, height: px(6), above: px(36), below: px(36),
    fill: gradient.linear(
      (color-red, 0%), (color-red, 33.33%), (color-blue, 33.33%),
      (color-blue, 66.66%), (color-yellow, 66.66%), (color-yellow, 100%), dir: ltr),
  )
...
#let conf(doc) = {
  ...
  set text(font: font-text, fill: color-ink, size: tx(size-body), lang: "zh", weight: 400)
  set par(
  ...
  show strong: it => box(
    fill: color-red, inset: (x: px(6)), outset: (y: px(3)),
  ...
  show heading.where(level: 3): it => block(
    ...
    fill: white, stroke: (left: px(3) + color-red), inset: (left: px(12), right: px(12), y: px(6)),
  )[
    #set text(font: hf(3, font-bold), size: tx(size-h3), weight: 700, fill: color-ink, tracking: 0.05em)
    #set par(leading: 0.45em)
    #upper(it.body)
  ]
  show heading.where(level: 4): it => block(above: px(24), below: px(12))[
    #set text(font: hf(4, font-text), size: tx(size-h4), weight: 700, fill: color-blue); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 5): it => block(above: px(20), below: px(10))[
    #set text(font: hf(5, font-text), size: tx(size-h5), weight: 700, fill: color-em); #set par(leading: 0.4em); #it.body
  ]
  show heading.where(level: 6): it => block(above: px(18), below: px(10))[
    #set text(font: hf(6, font-text), size: tx(size-h6), weight: 700, fill: color-em); #set par(leading: 0.4em); #it.body
  ]
  ...
  show quote.where(block: true): it => block(
    ...
    stroke: (left: px(6) + color-yellow, right: px(2) + color-blue, top: px(2) + color-ink),
    inset: (left: px(20), right: px(20), y: px(16)),
    { set text(size: tx(16), fill: color-ink); it.body },
  )

  show raw.where(block: false): set text(font: font-mono, size: tx(size-code), fill: color-blue)
  show raw.where(block: true): it => block(
    width: 100%, above: px(24), below: px(24),
    fill: color-panel, clip: true,
    stroke: (left: px(6) + color-red),
  )[
    #block(width: 100%, height: px(36), fill: rgb("#e0e0e0"), inset: (x: px(12)))[
  ...
  set list(marker: text(fill: color-red, weight: 800)[•], body-indent: px(8), spacing: px(8))
  set enum(
    numbering: n => box(
      width: px(24), height: px(24), fill: color-blue,
  ...
  set table(
    stroke: px(3) + color-ink, inset: px(16),
